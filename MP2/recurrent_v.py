# -*- coding: utf-8 -*-
'''This is file is a clone of the recurrent.py file found in 
anaconda\Lib\site-packages\keras\layers, with the exception of the LSTM class
being a variant of the standard LSTM function.
'''
...
#Variant 1 of GRU, no input signal
    ...
    def step(self, x, states):
        ...
        if self.consume_less == 'gpu':
            ...
            if self.consume_less == 'cpu':
                ...
                x_z = 0*K.dot(x * B_W[0], self.W_z) + self.b_z
                x_r = 0*K.dot(x * B_W[1], self.W_r) + self.b_r
                x_h = K.dot(x * B_W[2], self.W_h) + self.b_h
            else:
                raise ValueError('Unknown `consume_less` mode.')
            z = self.inner_activation(x_z + K.dot(h_tm1 * B_U[0], self.U_z))
    ...
    def get_config(self):
        config = {'output_dim': self.output_dim,
                  ...
                  'dropout_W': self.dropout_W,
                  'dropout_U': self.dropout_U}
        base_config = super(GRU1, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
    
class GRU2(Recurrent):
#Variant 2 of GRU, no input signal + no bias
    ...
    def step(self, x, states):
        ...
        if self.consume_less == 'gpu':
            ...
            if self.consume_less == 'cpu':
                ...
                raise ValueError('Unknown `consume_less` mode.')
            z = self.inner_activation(0*x_z + K.dot(h_tm1 * B_U[0], self.U_z))
            r = self.inner_activation(0*x_r + K.dot(h_tm1 * B_U[1], self.U_r))
    ...
    def get_config(self):
        config = {'output_dim': self.output_dim,
                  ...
                  'dropout_W': self.dropout_W,
                  'dropout_U': self.dropout_U}
        base_config = super(GRU2, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
    
class GRU3(Recurrent):
#Variant 3 of GRU, no input signal + no hidden unit signal
    ...
    def __init__(self, output_dim,
        ...
        if self.dropout_W or self.dropout_U:
            self.uses_learning_phase = True
        super(GRU3, self).__init__(**kwargs)
    ...
    def step(self, x, states):
        ...
        if self.consume_less == 'gpu':
            ...
            if self.consume_less == 'cpu':
                ...
                raise ValueError('Unknown `consume_less` mode.')
            z = self.inner_activation(x_z + 0*K.dot(h_tm1 * B_U[0], self.U_z))
            r = self.inner_activation(x_r + 0*K.dot(h_tm1 * B_U[1], self.U_r))
    ...
    def get_config(self):
        config = {'output_dim': self.output_dim,
                  ...
                  'dropout_W': self.dropout_W,
                  'dropout_U': self.dropout_U}
        base_config = super(GRU3, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
